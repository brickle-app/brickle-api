...
public class GetAllLeasingsQueryHandler : IRequestHandler<GetAllLeasingsQuery, IEnumerable<LeasingDto>>
{
    ...
    public GetAllLeasingsQueryHandler(ILeasingRepository leasingRepository, IUserLeasingAgreementRepository agreementRepository)
    {
        _leasingRepository = leasingRepository;
        _agreementRepository = agreementRepository;
    }

    public async Task<IEnumerable<LeasingDto>> Handle(GetAllLeasingsQuery request, CancellationToken cancellationToken)
    {
        IEnumerable<Domain.Entities.Leasing> leasings = await _leasingRepository.GetAllAsync();

        if (request.Active.HasValue)
        {
            leasings = leasings.Where(l => l.Active == request.Active.Value);
        }
